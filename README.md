...
# 같은 네트워크의 다른 기기에서도 접근하려면 --host 0.0.0.0 추가
...
1. 백엔드를 `--host 0.0.0.0`으로 실행합니다 (위 참고).
2. 프론트엔드는 `npm run dev`만 실행하면 이미 `0.0.0.0`에 바인딩됩니다.
3. 백엔드를 실행한 PC의 LAN IP를 확인합니다 (`ipconfig`의 IPv4 주소, 예: `192.168.0.10`).
4. 다른 기기의 브라우저에서 `http://192.168.0.10:5173` 으로 접속합니다.
